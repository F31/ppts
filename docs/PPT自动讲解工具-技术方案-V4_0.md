# PPT 自动讲解工具 — 完整技术方案 V4.0

> 版本：V4.0｜编制日期：2026-09-10  
> 基线：《PPT自动讲解工具-技术方案-V3.6.md》；本版聚焦用户明确指出的五个未完善问题：桌面端隐私模式的产品化边界、Web/移动端服务端上传处理的合理性评估、多租户对象存储隔离、可配置多后端存储与成本优化、是否引入向量化检索。其余章节结论沿用V3.6，未重新评审。  
> 定位：可从产品验证逐步演进到企业服务的开发设计基线。  
> 证据边界：本次未取得 go-pptx 源码及独立实测数据，相关能力仍按“待验证依赖”处理。新增的存储成本、向量检索技术描述已核对官方文档，具体计费和区域可用性以实际签约条件为准。
...
### 1.0.1 V3.6 → V4.0 本轮决策与变更（用户提出的五个待完善问题）

V4.0不重新评审V3.6已确认的架构方向，只针对用户明确指出的五处未完善内容给出结论，具体设计见对应章节。

| 用户提出的问题 | V3.6状态 | V4.0结论 | 对应章节 |
|---|---|---|---|
| 桌面端隐私保护，PPT文件不上传 | 11.4/11.7已有本地能力分级，但未形成用户可感知的独立开关 | 新增"隐私模式"作为桌面端显式产品开关，明确其能力边界与降级项，移动端不提供该模式 | 11.10（新增） |
| Web/移动端服务端上传处理是否合理 | 隐含云端处理为默认，未正面论证 | 论证云端处理是当前平台技术约束下的合理默认，同时给出留存最小化、处理后清除、区域选择三项缓解措施 | 13.1（新增） |
| 多租户用户资源文件隔离 | 12.1只覆盖数据库RLS，对象存储隔离未展开 | 新增对象存储键结构、签名范围、跨租户测试对应关系的独立设计 | 12.4（新增） |
| 存储可配置多后端与最优成本 | 4.3仅一行带过，未设计抽象接口和成本策略 | 新增`ObjectStore`端口与多后端适配器、按租户选择存储后端、生命周期分层降低成本 | 12.4、14.3（扩展） |
| 是否需要文档向量化 | 未讨论 | 不作为默认基础设施；仅在"全篇问答"等具体功能验证需求后，用pgvector而非独立向量数据库按需引入 | 6.6（新增） |
...
| 存储 | `ObjectStore`端口 + S3兼容多后端适配器（详见12.4/12.5） | 按租户可配置后端；单机开发可选本地文件适配器，多节点须共享存储；生命周期策略降低长期存储成本 |
...
### 6.6 是否引入文档向量化（V4.0 新增）

**结论：不作为默认基础设施；仅在具体功能明确需要"语义检索"而非"关键词检索"时，按需引入，且优先复用已有PostgreSQL而非新增独立向量数据库。**

需要先区分两类检索需求，二者不应该被同一个方案笼统覆盖：

| 需求 | 典型场景 | 适合的技术 | 是否需要向量化 |
|---|---|---|---|
| 关键词/结构化检索 | 3.3节P1"全文检索"——用户在自己的项目里按标题、术语、数字找到某一页 | PostgreSQL全文检索（`tsvector`/GIN索引）或同等的关键词检索 | **不需要**。这是精确匹配和词形变化匹配的场景，向量检索在这里既不更准确，也会引入不必要的嵌入调用成本和额外的召回噪声 |
| 语义/问答检索 | 6.5节P2"暂停提问"——用户提问的措辞和原文不完全一致，需要按语义相关性在全篇讲稿中找到答案依据 | 向量相似度检索 + 已有的来源锚点机制（6.1节） | **需要**，但只在该功能实际立项时才引入 |

**为什么不默认引入**：向量化需要额外的嵌入模型调用（有真实的Token成本）、额外的索引维护（讲稿分段修改后需要重新计算并失效旧向量，规则可以复用7.3节"增量失效规则"的思路），以及召回质量的持续评估（向量检索存在"看似相关实则跑题"的风险，需要配套的相关性阈值和人工可核验的来源锚点，不能直接把检索结果当成确定性事实）。在"暂停提问"功能尚未验证真实需求量之前引入这套机制，是为一个P2级、尚未排期的功能预先负担基础设施成本，不符合本方案一贯坚持的"以实际需求驱动基础设施引入"的原则（对照14.1节部署档位、10.5节NATS引入条件的判断逻辑）。

**技术选型建议（一旦"暂停提问"功能进入排期）**：优先使用 **pgvector**——PostgreSQL的开源向量扩展，支持精确和近似最近邻检索、多种距离度量，且可以和现有的业务数据表在同一个数据库实例、同一个事务里查询 [27]。这个选择直接对齐V3.6已经确立的"数据库任务表优先于独立消息队列""RLS优先于独立隔离基础设施"这一系列判断逻辑——向量检索的数据规模（每个项目的讲稿分段，通常几十到几百条）远达不到需要独立向量数据库（如Milvus/Qdrant/Pinecone）的规模量级，pgvector可以直接复用已有的PostgreSQL部署、已有的连接池、已有的RLS租户隔离机制（向量表同样是普通业务表，天然继承12.1节的行级安全策略），不需要为一个规模很小的检索需求单独运维一套向量数据库集群。

**触发独立向量数据库的信号**：单租户讲稿分段量级增长到百万级、查询延迟在pgvector的HNSW/IVFFlat索引调优后仍不达标、或者出现"跨租户共享知识库检索"这类pgvector的行级安全模型难以高效表达的场景，届时再评估迁移到独立向量数据库，不作为首版决策。
...
### 11.10 桌面隐私模式（V4.0 新增）

11.4节和11.7节已经分别给出了"各端能力与执行位置"矩阵和"离线等级"分级，两者组合起来事实上已经描述了一条"PPT文件不上传"的路径（L3桌面离线生成），但此前版本把它作为能力矩阵里的一格，而不是用户能直接理解和选择的产品功能。用户明确要求把它产品化，V4.0补上这一层。

**产品化为显式开关**：桌面客户端在项目创建或设置里提供"隐私模式"开关，而不是让用户去推断"哪些操作会联网"。开启后：

| 能力 | 隐私模式下的行为 | 与云端模式的差异 |
|---|---|---|
| PPT解析、备注提取 | 只经Go sidecar本地执行，文件不上传 | 云端模式下默认走服务端（11.4节） |
| 渲染 | 仅使用已认证的本地渲染能力包（LibreOffice等），不调用云端渲染 | 云端模式下默认云端渲染 |
| 讲稿生成（AI草稿） | **不可用**，除非用户显式配置本地可用的LLM推理能力（当前不作为首版承诺，见16.2节"不把……向量数据库设为基础依赖"同类原则，本地LLM同样不作为首版基础设施） | 云端模式下默认调用云端LLM |
| TTS语音合成 | 仅可用已认证的本地TTS能力包（如CosyVoice，8.2节已列为候选，需逐项核验模型权重与素材使用条件） | 云端模式下默认云端正式TTS供应商 |
| 数字人、多语言翻译等P2能力 | 不可用 | 这类能力天然依赖云端服务，隐私模式下没有替代路径 |

**核心纪律：隐私模式是"能力子集"而不是"功能降级的模糊承诺"**。界面上必须逐项列出隐私模式下"可用/不可用"，不能笼统宣传"隐私模式支持完整功能"——这与5.3节"不得把源文件动画XML仍在当成加入配音后动画顺序正确"、11.4节"不得因缺少本地模型而静默上传"是同一条纪律：任何能力边界都要显式声明，不能靠用户自己发现。

**移动端明确不提供该模式**：11.6节已经说明移动端不具备桌面sidecar能力（Tauri Shell插件在移动平台的限制），因此隐私模式是**桌面独有的产品能力**，移动端在产品文案上不能暗示同等能力，这一点在13.1节（Web/移动端上传合理性评估）里进一步展开。

**与本地能力包的关系**：隐私模式的实际可用程度取决于用户是否安装了对应的本地能力包（本地渲染器、本地TTS模型），11.7节已经要求"界面展示可用/未安装/不支持"，隐私模式开关本质上是把这些底层能力状态收敛成一个用户决策入口——用户开启隐私模式前，界面应该先检测并提示"当前本地能力包完整度"，避免用户开启后才发现大部分功能不可用。
...
### 12.4 对象存储隔离与可配置多后端（V4.0 新增）

12.1节的RLS只覆盖PostgreSQL里的结构化业务数据，源文件、渲染中间产物、音频资产、成品视频这些实际占用容量最大的内容存在对象存储里，隔离机制和数据库层是两套独立的防线，此前版本未展开，属于用户明确指出的缺口。

**端口抽象**：延续4.2节"对象存储与业务数据库分别使用`ObjectStore`和领域Repository"的既有判断，`ObjectStore`定义为独立端口：
...
业务代码只依赖这个接口，不直接引用任何云厂商SDK，这样"更换存储后端"或"同一部署里不同租户使用不同后端"都只是注入不同的实现，不改动上层逻辑。

**键结构与隔离**：所有对象键强制包含租户和项目前缀，不允许业务代码自行拼接绕过：

```
{tenant_id}/{project_id}/{source_revision|artifact_id}/{asset_type}/{asset_id}.{ext}
```

- 预签名下载/上传链接的生成必须校验调用方的租户上下文与键前缀一致，越权前缀直接拒绝签发，这一校验与12.1节的数据库RLS是两套独立生效的机制，不能用其中一个替代另一个；
- 15.3节已有的测试项"跨租户……对象签名"正是验证这条边界，V4.0在此基础上补充：签名校验逻辑本身也要有独立单元测试，不能只靠集成测试偶然覆盖到。

**多后端配置与选型**：

| 后端 | 适用场景 | 说明 |
|---|---|---|
| S3兼容对象存储（AWS S3/MinIO自建/阿里云OSS/腾讯云COS等） | 默认多租户共享部署 | 通过统一的S3兼容协议接入，各厂商在存储类别、区域和计费上有差异，接入前逐项核实 |
| 本地文件系统 | 单机开发/测试环境 | 不用于多节点生产部署，4.3节已注明"多节点须共享存储" |
| 客户自带存储（BYOS） | 企业客户明确的数据主权/合规要求 | 客户提供自己的存储账号和桶，产品侧只处理键结构和签名逻辑，不托管企业数据；这是12.3节"独立schema/数据库升级"之外，另一条面向企业客户的隔离升级路径，两者可以独立启用 |

后端的选择粒度到**租户级别**，通过配置而非代码分支决定某个租户的对象请求路由到哪个`ObjectStore`实现，这个配置本身作为12.1节租户策略的一部分持久化，不是部署时的全局静态配置——这是回应用户"可以配置选择多种存储方式"这一具体诉求的直接设计。

**加密**：默认使用云厂商的服务端加密（SSE）；对隔离要求更高的企业租户，可选启用信封加密（envelope encryption），即为该租户分配独立的数据加密密钥，由密钥管理服务（KMS）托管主密钥，这一层加密独立于12.1节的数据库层隔离和上文的键前缀隔离，是第三层纵深防御，不作为默认档位，按租户等级配置启用。

### 12.5 存储成本与生命周期策略（V4.0 新增）

延续14.3节"项目成本"公式，本节展开存储部分的具体成本优化设计，这是用户明确要求的"最优成本问题"。

**分层生命周期策略**：并非所有对象在项目全生命周期内都需要保持同一存储类别，参照AWS S3生命周期策略的通用做法——按对象年龄或访问模式自动在存储类别间转换或过期删除 [28]，本产品对不同资产类型设计不同的默认生命周期：

| 资产类型 | 活跃期存储 | 生命周期策略 | 依据 |
|---|---|---|---|
| 源PPT文件、当前revision的渲染页面图 | 标准/热存储 | 项目归档或删除前保持热存储，随时可能被重新打开编辑 | 7.1节"SourceRevision不可变"，随时可能被引用 |
| 中间音频母版（未压缩） | 标准存储 | 项目导出完成且用户确认后N天，转入低频访问类别；超过保留期且无进行中编辑，按13章删除策略清理 | 8.3节已区分"母版"与"压缩分发版本"，母版只在需要重新处理时才有价值 |
| 已完成的最终成品（MP4/带音频PPTX） | 标准存储，命中CDN | 按租户套餐的下载/分享有效期决定何时转入低频访问类别 | 9.1节"分享可过期/撤销" |
| 已失效的旧revision中间产物 | — | 按7.3节增量失效规则判定不再被任何当前引用的对象，进入删除队列 | 避免为已经不可达的中间产物持续付费 |

**去重降低成本**：7.1节`AudioAsset.content_hash`和`Artifact.hash`已经把内容哈希作为字段设计，V4.0明确要求实现层面据此做**跨revision、跨项目（同租户内）的对象去重**——文本和参数完全相同的分段音频不需要重复存储和重复调用TTS，直接引用已有的内容哈希对象，这既降低存储成本也降低供应商调用成本，是9.1节"局部重生成默认替换完整句群"场景下最直接的成本优化点。

**成本可见性**：`ApplyLifecyclePolicy`的执行结果和存储占用按租户汇总，接入14.2节已有的指标体系（"每项目成本"），让存储成本和14.3节的LLM/TTS成本口径一致地呈现给运营方，而不是只在云厂商账单里才能看到。

**不做的事**：不在首版为存储引入跨云厂商的多活复制或自建纠删码存储集群——这类能力和13章/14章反复强调的原则一致，只有在真实出现"某类租户明确要求存储层灾备等级高于对象存储厂商自身承诺"时才评估，首版依赖存储厂商自身的可用性和冗余能力（14.1节已注明"对象存储不同存储类别和区域的冗余能力不同，必须逐项核验"）。
...
### 13.1 Web/移动端服务端处理的合理性评估（V4.0 新增）

用户明确要求评估"Web端和移动端服务端上传后处理"这一默认设计是否合理，本节正面回答，不回避。

**结论：服务端处理是当前技术约束下的合理默认，而不是为了省事而选择的默认。** 理由基于平台的真实能力边界，而非"云端更简单"这类不充分的理由：

| 处理环节 | 浏览器/移动端能否本地完成 | 结论 |
|---|---|---|
| PPTX结构解析、备注提取 | 部分可行——go-pptx若具备WASM编译能力可以在浏览器本地完成轻量解析（此判断仍属5.1节"待验证依赖"，需go-pptx实测确认） | 可以作为**上传前的本地预处理**，用于提前发现异常页、缺失字体，减少无效上传，但不能替代下一步 |
| LLM讲稿生成、TTS语音合成 | 不可行 | 依赖云端模型服务，移动端和浏览器都不具备在本地以可接受的质量、延迟和电量成本运行同等能力模型的条件；8.2节已明确定位正式TTS供应商为云端服务 |
| 高保真渲染、视频编码 | 移动端不可行；浏览器可行性有限 | FFmpeg/渲染器是本地进程级依赖，11.6节已明确移动端不运行桌面式子进程；浏览器内视频编码性能和电量代价对移动网页不现实 |

也就是说，**"讲稿生成"和"语音合成"这两个产品核心能力天然要求服务端参与**，Web/移动端"上传后由服务端处理"不是可以绕开的实现选择，而是这两类AI能力目前的技术现实。11.4节的能力矩阵、11.7节的离线等级设计已经体现了这个判断，V4.0在此基础上补充的是**为什么这是合理的**，以及**在承认这个前提下如何降低隐私顾虑**。

**缓解措施**（不改变"必须上传"这个前提，但降低上传带来的风险）：

1. **处理后清除原文件**（新增可配置策略）：项目设置里增加"处理完成后删除源文件对象，仅保留派生的讲稿/音频/成片"选项，用户可以在生成完成后主动清除服务端保留的原始PPTX，只保留必要的派生产物；这与13章已有的"删除项目后按保留政策清理"是同一机制的提前触发版本，不是新增基础设施；
2. **最小化保留期限**（默认收紧）：即便不主动清除，源文件的默认保留期限也应该独立于派生产物单独配置，源文件可以设置比派生产物更短的默认保留期，降低长期存储敏感原始内容的暴露面；
3. **传输与静态加密**：TLS加密传输和12.4节的服务端加密是最基础的默认保障，企业客户可以叠加12.4节的信封加密选项；
4. **区域选择**：12.4节的多后端配置同时解决"存储在哪个地理区域"这一常见合规诉求，企业租户可以指定符合其数据主权要求的存储区域；
5. **诚实的能力边界说明**：产品界面上，当用户在Web/移动端上传文件时，应明确告知"此操作会将文件上传至云端处理"，而不是让用户直到查看隐私政策才发现——这是对11.10节"隐私模式"的呼应，即便Web/移动端不提供完整的隐私模式，也不应该在这一点上模糊处理。

**不做的事**：不承诺"Web/移动端也能像桌面隐私模式一样做到文件不出设备"——这在当前AI能力的部署现实下是不诚实的承诺，11.6节已经解释了移动端为什么不具备桌面sidecar能力，这个结论在V4.0保持不变。
...
首版共用React编辑器并分阶段交付Tauri桌面/移动，不建设Wails或独立Flutter工程，不自研完整Office渲染引擎，不把多Agent、独立部署的向量数据库或本地LLM推理设为基础依赖，不预建MySQL适配，不默认引入Kubernetes/Redis/NATS，不宣称任意动画和完整离线能力已具备。数字人能力本身不在暂缓之列（3.3/6节已作为P2独立扩展路径存在），暂缓的是"把它当成基础依赖"这一做法；若后续验证"暂停提问"等语义检索需求，优先用PostgreSQL的pgvector扩展而非独立向量数据库（6.6节）。上述能力均有已定义接入边界，后续通过独立ADR和验收添加。
...
| 014（V4.0新增） | `ObjectStore`端口 + 按租户可配置多后端存储，含生命周期分层降低成本 | 出现单一后端无法满足的合规/成本要求时增加新适配器实现，不改动业务逻辑 |
| 015（V4.0新增） | 桌面隐私模式作为独立产品开关，移动端不提供 | 移动端具备等效本地执行能力（当前判断不成立，见11.6节）时重新评估 |
| 016（V4.0新增） | 向量检索按需引入，优先pgvector，不作为默认基础设施 | 检索规模或跨租户共享检索需求超出pgvector合理范围时评估独立向量数据库 |
...
27. pgvector — [官方项目](https://github.com/pgvector/pgvector)：PostgreSQL向量相似度检索扩展，支持精确与近似最近邻检索。
28. AWS — [Transitioning objects using Amazon S3 Lifecycle](https://docs.aws.amazon.com/AmazonS3/latest/userguide/lifecycle-transition-general-considerations.html)：对象存储生命周期策略、存储类别转换的通用概念参考；具体到其他云厂商的对应能力需逐项核实。
