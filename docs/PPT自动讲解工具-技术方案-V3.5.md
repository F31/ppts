# PPT 自动讲解工具 — 完整技术方案 V3.5

> 版本：V3.5｜编制日期：2026-09-09  
> 基线：用户提供的《PPT自动讲解工具-技术方案-V3.md》，并补齐朗读、AI讲解、播放和导出全链路。  
...
V3.5 的推荐方案是：**Go 模块化单体 API + 独立 worker + PostgreSQL 持久化任务 + 对象存储；以统一讲解工程模型连接解析、讲稿、音频、字幕和导出；Web 优先交付，桌面及移动按需求推进。**

### 1.1 六维评审

| 维度 | V3 的有效设计 | 主要不足 | V3.5 调整 |
|---|---|---|---|
...
| Flutter 借 TS 桥接 Connect | 已有 Connect-Dart；应直接验证 Dart 客户端 [5] | 无需为了 API 引入 JS 桥 |
...
P0 为可用首版；P1 为产品化增强；P2 为独立扩展，版本号 V3.5 不意味着全部能力已经开发。
...
```mermaid
flowchart TD
  UI["Web / 后续桌面与移动"] --> API["Go API：认证、项目、讲稿、任务"]
...
| 层 | 推荐默认 | 原因与限制 |
|---|---|---|
| 编辑器 | React + TypeScript + Vite | 适合静态部署，并复用于Wails；公开营销站有SEO需求时另用Next.js |
| UI状态 | TanStack Query + 少量本地状态库 | 区分服务端缓存、编辑草稿和播放器状态 |
| API | Go + net/http + Connect；chi按路由需要引入 | 保留V3契约方向，减少重复框架 |
| 契约 | Protobuf + Buf lint/breaking + 生成客户端 | Go/TS优先；Flutter阶段验证Dart |
...
| 客户端 | 默认范围 | 推进条件 |
|---|---|---|
| Web/PWA | 全功能编辑、播放、导出管理；响应式审阅 | P0主端 |
| Wails桌面 | 复用React编辑器，连接云端；本地能力独立开关 | 真实的本地文件/隐私/离线需求 |
| Flutter移动 | 审稿、试听、项目管理、通知 | PWA无法满足后台音频、系统集成等需求 |
| PPT插件 | 备注同步、生成音轨和回写 | 验证用户长期在PowerPoint内工作的需求 |

Wails复用系统WebView [11]，但完整安装包还受模型、FFmpeg、渲染器和平台依赖影响，不能预先承诺远小于所有Electron方案。桌面稳定版本优先，未稳定主版本需独立评估。

### 11.4 离线能力是明确交付包

| 本地操作 | 必要条件 |
|---|---|
| 解析与讲稿编辑 | 通过验证的go-pptx、本地项目数据库 |
| 页面预览 | 本地渲染器+字体，或已有缓存页面图 |
| 朗读音频生成 | 预装可用本地TTS及模型/声音资产，或已缓存音频 |
| AI写稿 | 本地LLM及其运行资源，或用户手写稿 |
| 视频导出 | 本地FFmpeg及全部输入素材 |
| 完整离线首次运行 | 安装包/离线包已包含全部依赖，不在首次使用时联网补齐 |

桌面使用SQLite保存本地项目、任务与账本状态；共享领域规则和工程格式，独立实现本地持久化，不强行复用PostgreSQL SQL。联网同步由用户主动上传固定快照；首版冲突生成分支副本，不自动覆盖云端。浏览器WASM只作为局部解析/预检查优化，加载后还需验证内存、Worker隔离和大文件上限。
...
建议一个Go应用module，外部go-pptx按发布版本依赖；若共同开发可使用go.work，本地工作区不替代正式版本锁定。主要目录为 `cmd/api、cmd/worker、internal/project、internal/narration、internal/pipeline、internal/media、internal/tenant、internal/usage、internal/integrations、proto、migrations、web、testdata、docs/adr`。桌面与移动在实际启动时增加，不提前生成空项目。
...
假设一名后端、一名前端及兼职测试/产品支持，以下为顺序与工作量估算，部分可重叠；单人开发需要重新排期。go-pptx未完成、复杂PPT兼容和本地模型部署是主要不确定项。

| 阶段 | 参考时长 | 交付 | 放行条件 |
|---|---|---|---|
| G0：技术验证 | 1–2周 | go-pptx能力清单、样例解析/渲染、正式TTS、音频PPTX验证 | 明确认证客户端；不支持能力有替代路径 |
...
| G4：扩展包 | 独立估算 | 桌面离线、移动、数字人、动画、多语言、SSO高级集成 | 每项由真实需求、成本和支持矩阵放行 |

多租户字段、授权上下文、持久化任务和账本唯一性必须从G1建模；G3是加固与完整运营能力，不是等付费后再补基本隔离。配音PPTX若G0未通过，明确保留实验状态，先交付Web/MP4，不能以损坏文件完成里程碑。

### 16.1 首批开发任务

1. 固定第一批目标：中文PPTX、Web编辑、一个正式音色供应商、静态MP4；PowerPoint认证版本记录为测试环境配置。
...
首版不并行建设三端完整编辑器，不自研完整Office渲染引擎，不把多Agent、数字人或向量数据库设为基础依赖，不预建MySQL适配，不默认引入Kubernetes/Redis/NATS，不宣称任意动画和完整离线能力已具备。上述能力均有已定义接入边界，后续通过独立ADR和验收添加。
...
| 004 | Web优先，桌面/移动按需 | 明确离线、后台音频或系统能力缺口 |
...
11. Wails — [Introduction](https://wails.io/docs/introduction/)：Go/Web技术桌面应用与系统WebView。
