...
- 状态：已实现（2026-09-13：`migrations/0012_migrator_role.sql` 账号化 `ppts_migrator` 并转移现有表 owner；PG 门禁断言）
...
- `migrations/0012_migrator_role.sql` 创建 `ppts_migrator`，授予 schema create 权限，并将现有 public 表 owner 转移给迁移账号。
- 本地开发/测试库需要按同一角色模型初始化，确保 PG 门禁测试能复现"缺失上下文被拒绝"。
- 部署文档/CI 需继续固化"迁移账号 ≠ 运行账号"，后续迁移建议使用 `ppts_migrator` 执行。
