...
> 说明：G1 真实链路（上传→解析→原文讲稿→配音→播放 manifest）后端与 Web 已接通；剩余阻塞为页面图渲染（需 LibreOffice 环境）与正式 TTS 供应商验收（受凭据阻塞）。带 PG 的测试命令：`PPTS_TEST_DATABASE=... go test -tags=pg -p 1 ./...`。go-pptx 本地产库在 ADR-017 重构中间态触发 `BUG-001` 时，本地 ppts 门禁临时使用 `GOWORK=off`（发布 tag v1.0.1，与 CI 一致），不越界修改依赖仓库。
