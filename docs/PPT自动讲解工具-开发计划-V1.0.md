...
| 控制台前端重构（B0–B4） | ✅ 完成 | 按 `docs/PPT智能语音讲解平台-控制台设计方案-V1.5.md` 落地产品型控制台：自研 hash 路由（`web/src/router.tsx`）+ 会话/SessionContext（`session.ts`）+ 登录页（OIDC 优先、开发身份双端门控）+ `AppShell`（Sidebar 主导航：首页/讲解项目/任务中心/设置，Topbar 用户菜单与角色）；页面：`Home`（创作型首页：统计卡/最近项目/最近任务）、`Projects`（列表/新建/导入/归档/配音与页数探活）、`ProjectEditor`（三栏工作台：页面导航+讲稿编辑生成+配音属性(音色下拉/语速/Estimate)/播放/导出/冲突并排对比）、`Jobs`（列表/详情/取消/重试/活跃轮询）、`ProjectArtifacts`（配音快照+导出说明，不伪造成品列表）、设置五页（模型/成员/发音词典/用量/审计）；修复 Vite proxy（`/api`、`/ppts/object`、`/healthz`、`/debug/vars` → API）解决模型网关 JSON 报错根因；不引入 react-router，全量对接真实后端 API、不伪造数据 |
| Web 讲解工程导出（对应 `ARTIFACT_FORMAT_WEB_PROJECT`） | ✅ 实现 | `internal/artifact` 新增 `FormatWebProject`；`internal/api/export.go` 双向映射 proto `ARTIFACT_FORMAT_WEB_PROJECT` ↔ `web_project`（此前前端发送该格式但后端 `exportFormat` 未映射，导出恒返回 `unsupported export format`）；`internal/app/export.go` 新增 `packWebProject`：打包 zip（`timeline.json` + `subtitles.srt` + `subtitles.vtt` + `audio/<hash>.wav` 各片段）为可离线播放数据包；`internal/app/export_test.go` 新增 `TestExportHandlerPublishesWebProjectArtifact` 校验 zip 条目 |
...
| 2026-09-14 | V1.2 | 控制台前端重构（B0–B4）落地：按 `docs/PPT智能语音讲解平台-控制台设计方案-V1.5.md` 实现 hash 路由 + 会话/登录（OIDC 优先、开发身份双端门控）+ AppShell（Sidebar 主导航/用户菜单/角色）+ 首页/讲解项目/工作台三栏/任务中心/项目成品/设置五页；修复 Vite proxy 缺失 `/api` 致模型网关 JSON 报错；`router.tsx` 更名修正 `.ts` 下 JSX 解析（TS1005）；`npm run typecheck`/`build` 通过 |
| 2026-09-14 | V1.2 | Web 讲解工程导出（`ARTIFACT_FORMAT_WEB_PROJECT`）落地：修复"前端发送 WEB_PROJECT 但后端 `exportFormat` 未映射导致导出恒失败"的契约缺口；`internal/artifact.FormatWebProject` + `internal/api` 双向映射 + `internal/app.ExportHandler.packWebProject`（zip：timeline.json + 字幕 + 音频片段）；补 `TestExportHandlerPublishesWebProjectArtifact`，全量门禁绿 |
