...
### M10–M12：成稿数字合规——提示词同源 + 校验器归一化 + 第二轮定点修补（2026-09-22）【已实施】

**起因（先量后改）**：M9 之后「通过率提升」（方案 c）立项，于是先写临时探针，用**真实页面文字 + 像样的口播改写**
跑了 10 个用例，测「两轮校验不过」到底是模型不听话还是校验器误杀 —— 结果 **8/10 是误杀**，
即同一实体的**合法异写**被判成「删掉了原实体 + 新增了实体」。误杀全部是**归一化问题，不是阈值问题**，
因此有真实的、且**不放松安全门禁**的改进空间。探针用完即删（不留在仓库）。

**M11 校验器归一化（`internal/validation`）**：
- 抽数字前**先把日期区间挖成等长空白**：`2026-09-13` 原会被裸数字规则拆成 2026 / 09 / 13 三个噪声实体，
  而目标写成「2026 年 9 月 13 日」（旧正则对空格零容忍）时又整条匹配不上 → 双向误报。
  日期只认「年+月（+日）」，**不认单独年份**（单独的 `2026` 与「2026 年」都应等同为数量 2026）。
- 数字归一：千分位（`1,000` 原被切成 `1` 与 `000`）、前导零、小数尾随零（`3.0`→`3`）、全角→半角。
- **单位等价表（单一来源）**：`%↔pct`、`x↔倍`、`s↔sec↔秒`、`ms↔毫秒`、`min↔分钟`、`h↔hr↔小时`、`hz↔赫兹`；
  识别正则**由该表生成**（长单位优先，否则 `sec` 会被 `s` 抢先咬住）。
- **型号等价组**（按用户裁定「允许登记等价对」）：`k8s↔kubernetes`。只登记跨领域通用的写法；
  领域私有型号**不得猜测式登记** —— 宁可让一次真实改动落到可修的 degraded，也不要把改动静默放行。
- 计数前缀 `x8` / `×8` 折成裸数量（`A100 x8` 与「8 张 A100」同键）。这是**对称**归一，故 `x86` 与 `x64`
  仍会归一为 86 / 64 并判违规 —— 不放松门禁。
- 新增 `FormatEquivalenceHint()`，**提示词与校验器共用同一张表**。

**M10 提示词端（`draftInstructions`）**：加三条硬约束（原文实体逐一保留 / 原文没有的数字一律不得新增，
不自行编号或计数 / 型号与缩写保持原样），并拼接 `validation.FormatEquivalenceHint()`。
不自己在提示词里抄一份等价写法说明 —— 两处各写一份必然漂移，就会出现「模型照指令写、校验器仍拒绝」的
假失败，用户只看到整页回退原文，查不出是谁的问题（M1/M3 的教训）。

**M12 第二轮改定点修补（`draftText`）**：原先第二轮把**原文**+实体清单重发一遍，等于让模型从头重写，
第一版已写好的部分被丢弃、很可能换一种方式再错。改为以**第一版文案**为输入 + 逐条违规清单
（新增纯函数 `guardInstructions`），要求最小必要修改；两轮都不过仍回退原文并标 `degraded`（M9 不变量未变）。

**验证**：
- `internal/validation` 测试由 **2 项增至 8 项**：等价写法 10 例、真违规 7 例（守门）、序号不豁免、
  等价表与提示词覆盖一致（守门）、单位表无冲突、边界 8 例。**实测符合预期由 2/10 提升到 9/10**。
- 新增 `internal/app/scriptdraft_guard_test.go`：**用假 `TextRewriter` 把两轮流程本地跑通**
  （无 LLM / 无 DB / 无外部工具）—— 覆盖「第二轮输入必须是第一版文案」「第一轮通过时不发第二轮」
  「两轮都不过必须回退原文且标 degraded」「M11 的等价写法不再触发第二轮」。
  → 原先判断「M10/M12 涉及 LLM、无法本地验证」**只对了一半**：模型产出质量确实测不了，
  但**流程接线可以**，此前漏了这一点。
- 全量回归：`go build` / `go vet` / `go test -count=1 ./internal/...` 全绿（**26 包 ok / 0 FAIL / 16 项 SKIP**，
  与基线一致；`internal/api` 0 SKIP）。本次纯后端，未触及 `web/`，故无需 `tsc`/`vite`。
- 提交：`ad37b95`（M11）、`784ba3f`（M10+M12）。

**遗留（新登记，需另立里程碑）**：**阿拉伯数字 ↔ 中文数字**未处理（如源 `3 个` ↔ 目标「三个」）——
需中文数字解析，且有真实风险（「一键成稿」的"一"、"第三方"的"三"会被误当成数字）。

**2026-09-22 裁定：暂不做，保持现状**（用户明确选择"不做"）。据此不再停留在文档里，
而是**把缺口挂到回放语料上**：`internal/validation/replay/testdata/authored.json` 的
`auth-chinese-numeral-known-gap`（`3 个` → 「三个」，want=pass）当前必然报 1 例**误杀**——
缺口在每次回放里可见；一旦有人实现中文数字解析，该用例会从 block 翻成 pass，
**回放门禁立刻失败**并要求显式更新基线，即"缺口被补上了"必须是一个被审阅的事件。
（2026-09-22 实测确认：此例确实被拦下，且这就是 M11 剩余的 1 例误杀。）

### M13：gofmt 全仓收口（2026-09-22）【已实施】

`gofmt -l ./cmd ./internal` 此前报 **7 个文件**（`api/acl.go`、`api/acl_test.go`、`api/collab.go`、
`artifact/model.go`、`artifact/sqlite.go`、`db/db.go`、`pipeline/sqlite.go`），全部**早于 M1–M9**
（最后改动 `bbab840` / `2dc055e` / `8db7fc4` / `97139ee` / `e25efdf` / `434c587`），非本批引入。

**两处更正（对前面记录的修正）**：
1. **范围**：M9 曾记为「`internal/pipeline/sqlite.go` 不符合 gofmt」——那是因为当时只跑了
   `gofmt -l internal/pipeline/`，**漏报了另外 6 个**。验证命令的范围本身要核实。
2. **表述**：M1–M9 各节里的「gofmt 清白」指的是**本次改动的文件**清白，不是全仓清白。全仓直到 M13 才清白。

**坑（本次拦下的，重要）**：gofmt 对 Go 文档注释有「智能引号」重写规则，`api/acl.go` 注释里的
`` `$1 = ''` `` 会被改成 `` `$1 = ”` `` —— 这**不是格式调整，是把文档改错**（SQL 空串字面量被换成中文右引号）。
已把该行改写为「store SQL 中以空串入参表示不过滤」，释义不变且不再触发该规则。
**教训：格式化改动的 diff 必须逐行读过**，不能只看 `--stat` 就提交；
`git diff --ignore-cr-at-eol --stat` 只能排除行尾抖动，**排不掉内容被改**。

**验证**：`gofmt -l .` 全仓为空；`git diff --stat` 与 `--ignore-cr-at-eol` 统计一致（无行尾抖动，7 文件 26 增 19 删）；
`go build` / `go vet` / `go test -count=1 ./internal/...` 全绿（同上基线）。提交：`200c745`。

### M14：校验规则回放语料与基线门禁（2026-09-22）【已实施】

**动因**：M11 的效果是用一个**手写 10 例临时探针**估出来的（"符合预期 2/10 → 9/10"）。
手写探针有两个硬伤：样本小，且例子由作者挑选（偏向"我认为该通过"）。规则一旦被后人改动，
没有任何东西会拦住回归——而它是安全门禁。

**做法**：新建 `internal/validation/replay` 包 + `scripts/gen_replay` / `scripts/replay_eval`，
把 (原文, 成稿) 对固化成**可提交、可 review、可 gate** 的语料。

- 语料三来源：`generated`（从语料页文字机械生成等价改写/数字篡改，去主观性）、
  `authored`（人工域用例，补语料覆盖不到的日期/单位/千分位/型号）、
  `recorded`（真实模型输出的**判定基线快照**，不带期望；由 `ai_eval -dump-pairs` 录制）。
- **两条硬门禁**：① 漏放（应拦却放行）必须为 0——门禁失效是安全问题而非统计问题；
  ② 判定相对基线不得翻转，且新用例必须先登记基线（否则"加一条永远失败的用例"或
  "加一条把漏洞固化的用例"都能悄悄进仓库）。
- **尺子也要校准**：`Tag` 字段 + `TestCorpusCoverage` 断言语料覆盖 date/unit/thousands/
  fullwidth/multiplier-fold/model/number-*/measure 各族；某族用例被删光会直接失败。
- **不依赖 LLM/DB**：纯函数 + 提交的 JSON，直接进 `go test ./internal/...`。

**门禁有效性已验证**（不是"跑绿了"就算）：故意把 `RTX6000D ↔ A100` 登记为模型等价 →
立刻报「漏放 1 例 `auth-model-changed`（型号被换：RTX6000D → A100，必须拦）」，
并提示"修规则而不是改语料"。

**语料侧发现（重要）**：`testdata/corpus/generated` 的 100 页文字**几乎完全相同**
（"… page N：PCIe 5.0 x16 / 64 GB"）——只能验证读适配器，用于校验规则**几乎没有信息量**；
`scripts/ai_eval` 的通过率因此也只测了同一页 100 遍。真实金样在 sibling 仓库
（`E:/projects/go-pptx/testdata/corpus`，含 `H20 96GB 400W` 这类真实业务文本），
故生成器支持 `-corpus` / `GOPPTX_CORPUS`，并**优先取真实金样**。

**验证**：76 例（generated 60 / authored 16），误杀 0 / 漏放 0（语料随后在 M16 扩到 80 例、
再挂入中文数字缺口 1 例，见 M11 节；最终 81 例 / 误杀 1（即该缺口）/ 漏放 0）；`go test ./internal/...`
27 包 ok（含 +1 新包）/ 0 FAIL / 16 SKIP。提交：`9d51af6`。

### M15：等价表数据化 + 启动期表校验（2026-09-22）【已实施】

等价表从 Go 源码常量搬到内嵌 `rules.json`（`//go:embed`，无 IO/无 DB——安全门禁必须确定性），
并补上此前**完全缺失的表自检**。

`ValidateRules` 检查的都是"编译期看不见、运行期不报错、只表现为某实体被静默放行/误杀"的错误：
空 token、token 含空白、等价组只有 1 项、**同一 token 落在两个组**（`unitCanonical` 后写覆盖先写，
归一化结果会取决于声明顺序）、仅大小写不同的重复、子表全空。`mustLoadRules` 失败即 **panic**
（不静默降级）。`RulesVersion()` 供回放报告/基线留痕：这份判定是在哪版规则下得到的。

**为什么仍不进数据库**：等价表是安全门禁的底线，若可被配置改动，"同一份文本在两处结论不同"
的缺陷将不可复现。需要租户可配的是**领域型号别名**那一层，且必须只增不减（未做，见遗留）。

**验证（重构的验收标准是行为零变化）**：M14 的 76 例基线**零翻转**、误杀 0/漏放 0；
`rules_test.go` 覆盖默认表合法性 + 9 类表错误逐条必拦 + 合法表不得被误伤。
提交：`793a466`。

### M16：量词族分组参与判违规（2026-09-22）【已实施】

**动因（M14 语料实测出的真实漏放）**：「本章共 3 页」改写成「本章共 3 个部分」会被**放行**——
数字没变、事实却变了。但把所有量词纳入逐字比对又会误杀「3 个」↔「3 条」这类无害改写。
按裁定用**族分组**取两者之间：

- `measureGroups`：个/项/条/则/款/种/类 同族可互换；页、次、位/名、台、块… 各自成族
  （吃不准的单独成组，取保守侧）。量词组**允许只有一个量词**（不为凑数硬拉近似词进组）。
- 比较规则：同 Kind + 同 Text + **量词兼容**才配对；**任一侧缺量词视为兼容**
  （成稿常省略或改换说法，与"事实被改"不同），只有两侧都写了量词且异族才算冲突。
- 量词族名只在比较时使用，**不进** `FormatEntities`（那是给模型的实体串）；
  同一数量带不同量词时去重，避免指令出现 "number:3, number:3"。
- `FormatMeasureHint()` 与校验器同源，`draftInstructions` 带上它——量词已参与判违规，
  指令不说等于把整页回退原文的责任推给模型（M1/M3 的老问题）。

**验证**：既有 76 例**零翻转**（改动只影响带量词的场景）+ 新增 4 例 measure 用例判定与期望一致
（异族必拦 / 同族放行 / 缺量词放行），误杀 0 / 漏放 0；`FormatMeasureHint` 覆盖全部族的守门测试。
**顺带修正**：`ai_eval` 的评测指令原先没带等价说明，等于用**另一套规则**测模型——模型按生产指令
写「12 毫秒」会被按字面判违规，通过率失真且把问题错误归因给模型。已改为与校验器同源。
提交：`e2b2267`。

### 前端体验缺口盘点（2026-09-22）—— 只盘点，未立项

用户要求「先盘点再立项」。产出独立文档 **`docs/前端体验缺口盘点-2026-09-22.md`**，含：
9 处**静默错误压平**（违反 A26，全部逐条复核，给到 `文件:行`）、AuditPanel 缺 loading 判定、
7 处不可解释空态、公开区（`public.css`）**零窄屏适配**与数据表无窄屏规则、多处错误提示缺
`role="alert"`/`aria-live`、**已精确核实的 21 个死样式与 `ProjectEditor` 版本预览死代码**（2 个回调 + 4 个 state）。

文档同时记录了**三种会误报的统计口径**（`\b` 对 CSS 类名无效、i18n 动态 key 拼接、动态拼接的状态类名），
避免下次重复踩。**本轮未改任何前端代码**；是否立项、立几项由用户裁定。
...
| R-10 | 里程碑交付只做"单文件类型复核+`gofmt` 兜底"就提交，缺陷（编译阻塞、吞错假状态、未挂载路由）会跨里程碑累积 | 高 | 高 | 按「验证回路更新」在沙箱内跑通 `tsc -b` + `vite build` + `go build ./...` + `go vet ./internal/...` 后才提交；新增后端路由须核对**注册表**而非仅核对处理器函数。**2026-09-18 再犯**：`GET /artifacts`（B5-M2 跨项目成品库）handler 写完漏挂路由，请求落到 SPA 兜底返回 index.html，前端报 `Unexpected token '<', "<!doctype"`；已补路由 + 回归测试 `TestGlobalArtifactsRouteMounted` |
